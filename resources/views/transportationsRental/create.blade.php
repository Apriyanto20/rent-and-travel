<x-app-layout>
                                ...
                                <div class="grid grid-cols-4 gap-5">
                                    <div class="mb-5">
                                        <div
                                            ...
                                            <i class="fi fi-rr-id-card-clip-alt flex"></i> <span>Kategori Transportasi <span
                                                    class="text-red-500">*</span></span>
                                        </div>
                                        <select
                                            class="js-example-placeholder-single js-states form-control w-full m-6 border border-gray-300 rounded-3xl px-4"
                                            id="codeTransportation" name="codeTransportation" data-placeholder="Kategori Transportasi">
                                            <option value="">Pilih...</option>
                                            @foreach ($transportation as $m)
                                                <option value="{{ $m->codeTransportation }}">{{ $m->transportation }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="mb-5">
                                        ...
                                        <select
                                            class="js-example-placeholder-single js-states form-control w-full m-6 border border-gray-300 rounded-3xl px-4"
                                            id="codeMerk" name="codeMerk" data-placeholder="Status Pernikahan">
                                            <option value="">Pilih...</option>
                                            @foreach ($merk as $m)
    ...
    <script>
        document.getElementById('codeTransportation').addEventListener('change', function () {
            const selected = this.value;

            let type = '';
            if (selected.includes('BUS')) {
                type = 'bus';
            } else if (selected.includes('CAR')) {
                type = 'car';
            } else if (selected.includes('MOTOR')) {
                type = 'motor';
            }

            if (type) {
                fetch(`/generate-code-detail/${type}`)
                    .then(res => res.json())
                    .then(data => {
                        document.getElementById('codeDetailTransportation').value = data.code;
                    });
            } else {
                document.getElementById('codeDetailTransportation').value = '';
            }
        });
    </script>
    <script>
